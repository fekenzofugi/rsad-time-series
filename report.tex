...
Ghana’s climate is characterized by a strong seasonal cycle that directly affects both crop development and the usability of
optical remote sensing data. In the southern and central parts of the country, two main seasons are observed: a major rainy season 
(approximately April–July) and a minor rainy season
(September–October), separated by relatively drier periods. During the major rainy season, persistent cloud cover is common,
often resulting in near-continuous cloudiness at the time of peak vegetation growth. This poses a significant challenge for
optical satellite sensors such as Sentinel-2, as cloud contamination and cloud-shadow masking can severely reduce the 
availability of valid surface reflectance observations. Coincidentally, this period also overlaps with the main maize 
growing season, during which Fall Armyworm (Spodoptera frugiperda) infestations typically peak between June and July, 
when crops are at the vegetative and early reproductive stages most vulnerable to defoliation. As a result, the period 
of greatest agronomic relevance for detecting armyworm-induced stress is also the period with the poorest optical data 
availability. This seasonal coupling between rainfall, cloud cover, and pest dynamics highlights a key limitation of 
optical remote sensing in Ghana.

\section{Methodology}
Figure \ref{fig:abstract} illustrates the conceptual framework of FAW infestation detection. We first detect the anomaly in S2 time-series 
(FAW infestation), then we model an anomaly detection algorithm to identify such events, and 
finally we can create alerts for farmers to take action.
...
We are proposing a AD time-series model that can detect pest infestations in agricultural
...
We can extend the result to more dates, in periods before and after the infestation event, to
observe the full phenological cycle of the maize crop. 

\subsection{Spatial Analysis of FAW Infestation Patterns}
Compute ndvi series for each pixel, compute mean and variance, pest can present a higher variance. 
plot mean, median, variance maps.
